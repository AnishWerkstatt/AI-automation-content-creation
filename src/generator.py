...
def generate_curriculum(previous_titles=None):
    ...
    try:
        ...
        prompt = f"""
        You are an expert AI educator. Generate a curriculum for a YouTube series called 'AI for Developers'.
        {history}
...
def generate_lesson_content(lesson_title):
    ...
    try:
        ...
        model = genai.GenerativeModel('gemini-1.5-flash')
        prompt = f"""
        You are creating a lesson for the 'AI for Developers' series. The topic is '{lesson_title}'.
...
def generate_visuals(output_dir, video_type, slide_content=None, thumbnail_title=None, slide_number=0, total_slides=0):
    ...
    if not is_thumbnail:
        ...
        footer_height = int(height * 0.06)
        draw.rectangle([0, height - footer_height, width, height], fill=(25, 40, 65, 200))
        draw.text((40, height - footer_height + 12), f"AI for Developers", font=footer_font, fill=(180, 180, 180))
